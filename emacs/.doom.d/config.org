...
#+latex_header_extra: \setromanfont{Linux Libertine O}
#+latex_header_extra: \setmonofont{Sarasa Mono CL}
...
**** Org-modern, Org-appear, Org-fragtog, Org-CSL-activate
...
**** Setting up previews using =imagemagick=
For this to show properly:

#+begin_src emacs-lisp
(setq org-latex-create-formula-image-program 'imagemagick)
#+end_src

Remember that you have to comment out this line in =/etc/ImageMagick-6/policy.xml=:
...
(use-package! laas
  ...
  (aas-set-snippets 'laas-mode
    ...
    :cond (lambda nil (and (not (texmathp)) (memq (char-before) '(?\n ? ))))
