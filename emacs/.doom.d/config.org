...
#+latex_header_extra: \setmonofont[Scale=0.9]{Sarasa Mono CL}
...
This is particularly super useful for multitasking with multiple windows open.
...
It's even better that it allows us to cherry-pick glyphs to build the perfect style variant to use.
I build my own version of Iosevka, based on the Ubuntu Mono style variant (SS12).
This style brings me that nostagic feel of my first linux distribution.
The rounded letters (e.g. =l=, =m=, =n=, =i=, =j=,...) bring a stylish yet comfy looks.
