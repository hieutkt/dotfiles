...
It calls the button ~Ctrl~ on modern keyboards as ~C~ and the ~Alt~ button as ~M~.
By default emacs, you copy text by pressing ~M-w~ (read: ~Alt + w~ on a modern keyboard ), kill (cut) by ~C-w~ (~Ctrl + w~), and paste by ~C-y~ (~Ctrl + y~).
...
This document describes how I set up my Emacs, in [[https://en.wikipedia.org/wiki/Literate_programming][literate programming]] style, from a text format closely related to Emacs called [[https://orgmode.org/][Org-mode]].
